...
drawLink = (shapelist, link, end1, end2, worldShape, ctx, fontSize, font, isStamp = false) ->
  if not link['hidden?']
    ...
    getWrappedLines(x1, y1, x2, y2, worldShape, wrapX, wrapY).forEach(drawLinkLine(shapelist, worldShape, link, adjustedThickness, ctx, isStamp, fontSize, font))

drawLinkLine = (shapelist, worldShape, { color, size, heading, 'directed?': isDirected, shape: shapeName, label, 'label-color': labelColor },
                thickness, ctx, isStamp, fontSize, font) => ({ x1, y1, x2, y2 }) =>
    ...
    (line) =>
      ...
      if visible or isMiddleLine
        ...
        if visible
          ...
          drawSubline(offsetSubline, dashPattern, thickness, color, isCurved, controlX, controlY, ctx, worldShape.onePixel)

        if isMiddleLine
          if isDirected and size > (.25 * worldShape.onePixel)
            drawLinkShape(x2, y2, controlX, controlY, heading, color, thickness, shape, shape, ctx, worldShape.onePixel)
